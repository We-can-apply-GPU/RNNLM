...
time ./rnnlm -train train -valid valid -rnnlm model -hidden 50 -rand-seed 1 -debug 2 -class 500 -bptt 4 -bptt-block 10 -direct-order 3 -direct 2 -binary

#ngram model is trained here, using SRILM tools
$srilm_path/ngram-count -text train -order 5 -lm templm -kndiscount -interpolate -gt3min 1 -gt4min 1
$srilm_path/ngram -lm templm -order 5 -ppl test -debug 2 > temp.ppl
